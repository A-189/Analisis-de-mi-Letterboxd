---
title: "Analisis de mis Vistos en Letterboxd"
author: "A-189"
date: "2025-11-13"
...
## Análisis de mis Vistos en Letterboxd
...
```{r}
as.character(df_letterboxd_watched$letterboxd_uri)
sapply(df_letterboxd_watched, class)
```
...
```{r}
df_watched_separados <- df_letterboxd_watched_con_urls |>
  mutate(
...
```{r}
df_watched_genres <- df_watched_separados |>
...
```{r}
df_watched_themes <- df_watched_separados|>
...
```{r}
df_watched_nanogenres <- df_watched_separados |>
...
```{r}
df_watched_genres_con_conteo <- df_watched_genres |>
  drop_na() |>
  group_by(genres) |>
  summarise(n = n())
```
...
```{r}
df_watched_themes_con_conteo <- df_watched_themes |>
  drop_na() |>
  group_by(themes) |>
  summarise(n = n())
```
...
```{r}
df_watched_nanogenres_con_conteo <- df_watched_nanogenres |>
  drop_na() |>
  group_by(nanogenres) |>
  summarise(n = n())
```
...
```{r}
as.numeric(df_watched_genres_con_conteo$n)
sapply(df_watched_genres_con_conteo, class)
```

**Temáticas:**

```{r}
as.numeric(df_watched_themes_con_conteo$n)
sapply(df_watched_themes_con_conteo, class)
```

**Nanogéneros:**

```{r}
as.numeric(df_watched_nanogenres_con_conteo$n)
sapply(df_watched_nanogenres_con_conteo, class)
```
...
```{r}
grafico_watched_genres <- df_watched_genres_con_conteo |>
  drop_na() |>
  filter(genres != "") |>
  top_n(25) |>
  ...
  labs(title="Películas por Género - Vistas",
       x="Géneros",
       y="Cantidad") +
  theme_minimal() +
...
```{r}
grafico_watched_themes <- df_watched_themes_con_conteo |>
  filter(themes != "") |>
  top_n(25) |>
  ...
  labs(title="Películas por Temática - Vistas",
       x="Temáticas",
       y="Cantidad") +
  theme_minimal() +
  coord_flip()
...
```{r}
grafico_watched_nanogenres <- df_watched_nanogenres_con_conteo |>
  filter(nanogenres != "") |>
  top_n(25) |>
  ...
  labs(title="Películas por Nanogénero - Vistas",
       x="Nanogénero",
       y="Cantidad") +
  theme_minimal() +
  coord_flip()
